...
- **Dedicated Screenshot Gallery:** Serves a beautiful, interactive gallery at `/screenshots` that scans the console's `/Nintendo/Album` folder. The screenshots and videos are sorted chronologically by date (newest first). The interface automatically decodes the Nintendo Switch screenshot filename structure (`YYYYMMDDHHMMSS00-TITLEID.ext`) to show formatted human-readable dates (e.g., `YYYY-MM-DD HH:MM:SS`) and looks up the Title ID to retrieve the game's actual display name. It also features built-in video playback controls for MP4 captures.
- **Tools Menu Integration:** Offers dedicated fast-launch tiles directly in the console's **Tools** menu: **Start Web Server** (shares the SD Card root `/`) and **Screenshots** (shares the SD Card and links the landing page directly to the Screenshot Gallery), removing the need to open the File Browser.
