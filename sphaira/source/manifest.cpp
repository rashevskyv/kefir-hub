...
bool path_equal(std::string_view lhs, std::string_view rhs) {
    return lhs.length() == rhs.length() && !strncasecmp(lhs.data(), rhs.data(), lhs.length());
}
...
bool is_shared_file(const Manifest& m, const std::string& owner_id, const std::string& file) {
    ...
    for (const auto& [id, comp] : m.components) {
        ...
        for (const auto& other_file : comp.files) {
            if (path_equal(path, normalize_path(other_file).toString())) {
                return true;
            }
        }
    }
