...
| [x] 6.3 | `ui/menus/filebrowser_ops.cpp` | Heavy `FsView` operation bodies (same class, second TU — this is fine): `InstallFiles`, `UnzipFiles`, `ZipFiles`, `UploadFiles`, `ShareFolder`, `OnDeleteCallback`, `OnPasteCallback`, `CheckIfUpdateFolder`. UI/navigation methods (`Update`, `Draw`, `Scan`, `Sort`, selection) stay in `filebrowser.cpp`. |
