...
**Статус: виконано у v0.13.206. Фікс рев'ю у v0.13.207:** `SetMtpEnable(true)` ігнорував результат `haze::Init()` — після вимкнення останнього сховища тумблер MTP і конфіг лишалися «On» при незапущеному сервері. Тепер при провалі `Init()` опція відкочується у `false` (тумблер чесно показує Off, нотифікація «No MTP storages enabled» пояснює причину), `RegisterMtpCallbacks()` викликається лише при успішному старті; прибрано задубльований `#include` у haze_helper.cpp.
...
## Частина IV: Файловий менеджер — швидке перемикання джерел

### Крок S8: Пункт «Sources» у верхньому контекстному меню File Browser

**Факт.** Перемикання джерела (SD / stdio-носії USB-HDD / SMB-шари) вже реалізоване і працює — `FsView::DisplayAdvancedOptions()` ([filebrowser.cpp:1041–1125](sphaira/source/ui/menus/filebrowser.cpp:1041)), пункт `Mount` (SidebarEntryArray, будує список із `location::GetStdio()`, `FS_ENTRIES`, `location::Load()` для SMB). Проблема суто UX: цей пункт захований на другому рівні — головне контекстне меню → `Advanced` (підменю) → `Mount` (ще один пікер). Швидкого переходу «одна кнопка — список джерел» немає.

**Мета.** Винести перемикання джерела на верхній рівень контекстного меню File Browser (той самий рівень, де зараз `Compress`, `Advanced` тощо — будується десь біля [filebrowser.cpp:1035](sphaira/source/ui/menus/filebrowser.cpp:1035)), під пункт `Source`/`Sources`_i18n, без проміжного заходу в `Advanced`.

**Кроки:**
1. У побудові головного контекстного меню (там, де зараз додається `adv_entry = options->Add<SidebarEntryCallback>("Advanced"_i18n, ...)`) додати окремий пункт `"Source"_i18n`, що відкриває той самий `SidebarEntryArray` зі списком джерел — переиспользувати логіку побудови `fs_entries`/`mount_items` і callback вибору з `DisplayAdvancedOptions()` (винести в спільну приватну функцію `FsView::ShowSourcePicker()`, яку викликають і новий пункт, і (за бажанням) залишений старий `Mount` в Advanced — не дублювати код).
2. Список джерел має включати: microSD, усі підключені stdio-носії (USB/HDD, `location::GetStdio()`), збережені SMB-локації (`location::Load()`, `url.rfind("smb://")`). Якщо в проєкті буде або вже є FTP/WebDAV-джерело для файлового менеджера (наразі не знайдено — FTP-код у `ftp_menu.cpp`/`ftpsrv_helper.cpp` — це FTP-сервер на самій консолі, не клієнт-джерело для File Browser) — до списку не додавати, доки такого джерела не існує; не вигадувати нову інтеграцію в межах цього кроку.
3. Поточне активне джерело — позначити в списку (наприклад, префікс/маркер), щоб було видно, що зараз змонтовано в цій панелі (ліва/права — `ViewSide`).
4. i18n: ключ `"Source"_i18n` (або `"Sources"_i18n`) в `en.json` і `uk.json`; тултип на кшталт «Quickly switch this pane's file source»_i18n.
5. Версія в `sphaira/CMakeLists.txt`, запис у `walkthrough.md`.

**Перевірка:** відкрити контекстне меню File Browser → пункт джерела видно одразу (без заходу в Advanced) → вибір SD/USB миттєво перемикає панель; вибір SMB-локації йде через той самий Connecting-progressbox, що й раніше; поведінка `Advanced → Mount` (якщо залишений) не зламана; дубльованого коду побудови списку джерел немає (спільна функція).
...
| 8 | S8 (швидке перемикання джерел у File Browser) | малий/середній | незалежний |
