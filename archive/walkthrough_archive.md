# Архів змін (Walkthrough Archive)
...
### Завдання
Усунути зауваження аудиту щодо Фази 12:
1. **Усунення дуплікації**: Видалити копії функцій `IsImagePath` та `JsonEscape`, які залишилися в анонімному просторі назв `web.cpp` після їх перенесення у `web_http.cpp`.
2. **Формат закінчень рядків**: Повернути закінчення рядків файлу `web.cpp` до формату UNIX (LF) замісті Windows (CRLF), щоб відновити коректну історію `git blame` та уникнути повного перезапису файлу у diff.

### Підхід
1. Видалено визначення `IsImagePath` та `JsonEscape` з [web.cpp](file:///d:/git/dev/sphaira/sphaira/source/web.cpp).
2. За допомогою Python-скрипта конвертовано закінчення рядків [web.cpp](file:///d:/git/dev/sphaira/sphaira/source/web.cpp) назад у LF.
3. Проведено верифікаційну збірку через WSL для підтвердження відсутності помилок.
4. Оновлено версію програми у [CMakeLists.txt](file:///d:/git/dev/sphaira/sphaira/CMakeLists.txt) до `0.13.193`.

### Результати тестування
* Проект успішно збирається під WSL за допомогою cmake, файл `kefir-hub.nro` успішно згенеровано.
...
### Завдання
Виправити дефекти рефакторингу Фази 10 та Фази 11, пов'язані з втратою важливих коментарів під час декомпозиції коду:
1. **Фаза 10 (`save_menu.cpp`)**: Відновлено три втрачені коментарі в `save_menu_detail.cpp`.
2. **Фаза 11 (`cheat_game_select_menu.cpp`)**: Відновлено три втрачені коментарі в `cheat_download_menu.hpp` та `cheat_download_menu.cpp`.

### Підхід
1. Перевірено та зафіксовано наявність усіх шести відновлених коментарів у відповідних файлах реалізації та заголовках.
2. Оновлено документацію проекту (`walkthrough.md` та `task.md`), оновлено версію програми в `CMakeLists.txt` до `0.13.191`.
3. Виконано верифікаційну збірку проекту через WSL для підтвердження відсутності помилок.

### Результати тестування
* Проект успішно збирається під WSL (`cmake --build build/ReleaseWithInstall -j16`), файл `kefir-hub.nro` успішно згенеровано.
...
### Завдання
Виправити регресію стилю діалогового вікна підтвердження `HoldConfirmBox`:
- Раніше вибір між Legacy-лейаутом (820x468, з текстом «Hold A for 3 seconds to continue») та Compact-лейаутом (770x384, з текстом «Hold A to continue») відбувався неявно шляхом порівняння часу утримання кнопки (`hold_seconds != 3.0f`). Це призводило до регресії, коли виклик з рівно 3.0 секундами утримування, але який мав бути компактним (наприклад, при помилці активації модуля вентилятора), мовчки перемикався на Legacy-лейаут.
- Необхідно розділити поняття "тривалість утримання" та "стиль вікна" на незалежні змінні.

### Підхід
1. **Зміни у `HoldConfirmBox`**:
   - Додано прапорець `m_compact` у [hold_confirm_box.hpp](file:///d:/git/dev/sphaira/sphaira/include/ui/hold_confirm_box.hpp).
   - Оновлено конструктори у [hold_confirm_box.cpp](file:///d:/git/dev/sphaira/sphaira/source/ui/hold_confirm_box.cpp): конструктор з двома аргументами ініціалізує `m_compact = false`, а конструктор з трьома аргументами (який викликається з меню налаштувань та вентилятора) завжди ініціалізує `m_compact = true`.
   - Замінено всі розгалуження в `Draw()` та конструкторах з порівняння `m_hold_seconds != 3.0f` на логічне значення `m_compact`.
2. **Версіонування**:
   - Версію програми оновлено до `0.13.188`.
