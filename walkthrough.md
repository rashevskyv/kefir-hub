# Результати: Screensaver Analog Stick Interactive Controls (v0.13.430)

## 1. Інтерактивне переміщення скрінсейвера лівим стіком
- **Вільне переміщення:** Лівий аналоговий стік тепер дозволяє вільно пересувати весь блок скрінсейверу по екрану Switch (як у грі) зі швидкістю 450 px/s та плавним обмежуванням за межами екрану.
- **Обробка в `Screensaver::Update()`:** Додано розрахунок зміщення `m_user_offset_x` та `m_user_offset_y` без пробудження скрінсейверу.

## 2. Керування яскравістю та швидкістю правим стіком
- **Яскравість (Правий стік вгору/вниз):** Дозволяє плавно змінювати яскравість екрану консолі в реальному часі через `lblSetCurrentBrightnessSetting` / `lblApplyCurrentBrightnessSettingToBacklight` та зберігати значення.
- **Швидкість дрейфу (Правий стік вліво/вправо):** Дозволяє динамічно змінювати множник швидкості автоматичного плавання скрінсейверу (`m_drift_speed_mult`).

## 3. Зчитування аналогових стіків
- **Розширення `Controller`:** До структури `Controller` (`types.hpp`) додано поля `m_stick_l` та `m_stick_r` (`HidAnalogStickState`).
- **Опитування в `App::Poll()`:** `App::Poll()` зчитує `padGetStickPos(&m_pad, 0)` та `padGetStickPos(&m_pad, 1)`.

## 4. Версіонування
- **Ітерація версії:** Піднято версію проекту в `sphaira/CMakeLists.txt` до `0.13.430`.
