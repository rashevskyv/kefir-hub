...
## v0.13.225 — Оновлення мовних пакетів, кругова навігація та фільтрація варіантів перекладу

### Завдання
1. Додати підтримку кругової (циклічної) навігації у підменю налаштувань (SoftwareMenu, DbiMenu, KefirSettingsMenu, ThemesMenu, TranslateMenu).
2. Реалізувати динамічну зміну тексту кнопки завантаження мовних пакетів на "Update language packs" та опису на "Update the UltraHand language package list." за умови, що переклади вже завантажені (існує файл `TRANSLATE_PACKAGE`).
3. Додати фільтрацію доступних варіантів перекладу інтерфейсу відповідно до мови та регіону консолі (запит через сервіси `setGetSystemLanguage` та `setGetRegionCode`). Якщо жоден варіант не підходить, показувати діалогове вікно-попередження (`OptionBox`) зі списком усіх наявних варіантів.
4. Оновити файли локалізації (14 JSON файлів) новими ключами перекладу.
5. Виправити виявлену помилку компілятора GCC 16 у `save_menu.cpp` (заміна `auto` у параметрах лямбди `m_list->Draw` на конкретні типи `NVGcontext*`, `Theme*`, `Vec4`, `s64`).

### Реалізація
1. **Кругова навігація у підменю**:
   - У [settings_menu.cpp](file:///d:/git/dev/sphaira/sphaira/source/ui/menus/settings_menu.cpp) в конструкторах класів `SoftwareMenu`, `DbiMenu`, `KefirSettingsMenu`, `ThemesMenu` та `TranslateMenu` додано виклик `m_list->SetWrap(true);`, що дозволяє списку автоматично переходити до початку/кінця при прокрутці.
2. **Динамічний статус завантаження пакетів**:
   - У `BuildTranslateItems()` додано перевірку існування файлу `TRANSLATE_PACKAGE` за допомогою `fs::FileExists()`. За наявності файлу замість `"Download language packs"_i18n` відображається `"Update language packs"_i18n`, а замість `"Download the UltraHand language package list."_i18n` — `"Update the UltraHand language package list."_i18n`. Текст процесу завантаження також динамічно перемикається між `"Downloading..."` та `"Updating..."`.
3. **Фільтрація за мовою та регіоном**:
   - Додано допоміжну функцію `IsOptionApplicable`, яка зіставляє мову (`SetLanguage`) та регіон (`SetRegion`) консолі з назвою або директорією варіанта перекладу.
   - Регіональні коди консолі `SetRegion_HTK` (Hong Kong / Taiwan / Korea) використовуються для коректної перевірки варіантів для Кореї та Тайваню.
   - Якщо після фільтрації список сумісних варіантів порожній, користувачеві виводиться `OptionBox` із детальним списком усіх доступних варіацій перекладу та вимогою вибрати сумісний варіант у налаштуваннях консолі.
4. **Сумісність з GCC 16 (devkitPro)**:
   - У [save_menu.cpp](file:///d:/git/dev/sphaira/sphaira/source/ui/menus/save_menu.cpp) у виклику `m_list->Draw` параметри лямбда-функції замінено на конкретні типи `NVGcontext* vg, Theme* theme, Vec4 v, s64 pos`, що усунуло помилку перевантаження та виведення типів шаблону `std::is_invocable` у GCC 16.
5. **Оновлення локалізації**:
   - Написано та виконано Python-скрипт `scratch/update_i18n.py`, який додав нові ключі локалізації для 14 підтримуваних мов у `assets/romfs/i18n/`.
6. **Версія та збірка**:
   - Версію програми у [CMakeLists.txt](file:///d:/git/dev/sphaira/sphaira/CMakeLists.txt) підвищено до `0.13.225`.
   - Проект успішно скомпільовано під WSL, бінарний файл `kefir-hub.nro` успішно згенеровано.

## v0.13.224 — Виправлення крашу та покращення WebDAV інтеграції

### Завдання
1. Усунути Use-After-Free краш / Data Abort (fatal error 0xFFE / `std::abort()`), який відбувався при спробі додати мережеву локацію без префікса протоколу (наприклад, хост `dev.customfw.xyz`).
2. Виправити збій узгодження імен та URL-адрес у налаштуваннях автосинхронізації сейвів WebDAV (через що вибране джерело ніколи не підсвічувалося в меню та не розпізнавалося при синхронізації сейвів).
3. Додати додатковий захист від вильоту (розіменування невалідного індексу) у меню ігор та додатків при порожніх списках.

### Реалізація
1. **Виправлення Static Initialization Order Fiasco та minIni**:
   - У `location.cpp` повністю видалено глобальну змінну `location_path` типу `fs::FsPath`, яка ініціалізувалася за допомогою `paths::LOCATIONS`. Через невизначений порядок ініціалізації глобальних змінних (Static Initialization Order Fiasco) вона ініціалізувалася сміттям (оскільки `paths::LOCATIONS` була ще порожньою `std::string`). Замість неї у всіх викликах `minIni` (`ini_puts`, `ini_putl`, `ini_browse`) тепер використовується `paths::LOCATIONS.c_str()`, що гарантує валідність шляху на момент виклику.
   - У колбек `cb` для `ini_browse` додано перевірку на `nullptr` для параметрів `Section`, `Key` та `Value`. Це захищає від Data Abort крашу, якщо у файлі `locations.ini` зустрічаються коментарі, порожні рядки чи секції без ключів.
2. **Єдине джерело істини для WebDAV Saves**:
   - У `app.hpp` та `app_settings.cpp` додано метод `App::GetWebdavUrlName()`, який повертає назву вибраного джерела безпосередньо з `config.ini` (як рядок, збережений у `webdav_url`).
   - Переписано методи `App::GetWebdavUrl()`, `App::GetWebdavUser()` та `App::GetWebdavPass()`. Тепер вони спочатку шукають назву активного джерела у `locations.ini` за допомогою `location::Load()`. Якщо збіг знайдено — повертають URL, логін та пароль цього джерела. Якщо ні (старий формат) — повертають значення безпосередньо з `config.ini` (забезпечуючи зворотну сумісність).
   - У `app_display_options.cpp` (`DisplayWebdavOptions()`), `settings_menu.cpp` (`BuildSourcesCategoryItems()`) та `save_menu_ops.cpp` (`BackupSaves()`) оновлено порівняння активного джерела: тепер порівняння йде за іменем через `App::GetWebdavUrlName()`, а не за нормалізованим URL, що вирішило проблему з підсвічуванням вибраного джерела в меню WebDAV налаштувань.
3. **Безпека меню при порожніх списках**:
   - У `game_menu.cpp` та `homebrew.cpp` у методах `SortAndFindLastFile()` додано перевірки на порожній список. Якщо списки порожні, методи оновлюють UI та достроково виходять, уникаючи небезпечного доступу `m_entries[m_index]` чи `GetEntry().path` поза межами масиву.
4. **Збірка та деплой**:
   - Версію проекту підвищено до `0.13.224` у `CMakeLists.txt`. Проект успішно скомпільовано під WSL, бінарний файл `kefir-hub.nro` успішно скопійовано на SD карту (`H:\switch\kefir-hub.nro`).

## v0.13.223 — Виправлення Use-After-Free крашу при додаванні джерел

### Завдання
1. Виправити Use-After-Free краш (чорний екран Atmosphere з fatal error `std::abort() called (0xffe)`), який відбувався при спробі додати нову мережеву локацію (WebDAV, FTP, HTTP, SMB) через меню `Settings -> Sources`.

### Реалізація
1. **Виправлення крашу**:
   - У `filebrowser.cpp` в функції `AddNetworkLocationInteractive` замінено виклики `App::PopToMenu()` на `App::Pop()` для всіх 4 протоколів (Samba, WebDAV, FTP, HTTP).
   - **Причина крашу**: `App::PopToMenu()` закривав стек екранів до самого головного меню (MainMenu), що призводило до передчасного знищення об'єкта `SettingsMenu` з пам'яті. Коли після цього асинхронно викликався колбек `on_success()` (який намагався оновити список джерел через `menu->OnFocusGained()`), відбувалося звернення до вже знищеного об'єкта `menu` (розіменування дикого вказівника / Use-After-Free).
   - Використання `App::Pop()` замість `App::PopToMenu()` закриває виключно `PopupList` вибору протоколу, зберігаючи `SettingsMenu` або `FileBrowser` активними на стеку, що повністю усуває краш і забезпечує миттєве візуальне оновлення списку.
2. **Збірка**:
   - Версію підвищено до `0.13.223` у `CMakeLists.txt`. Проект успішно скомпільовано під WSL.

## v0.13.222 — Єдине джерело істини для WebDAV та резервного копіювання сейвів

### Завдання
1. Реалізувати єдине джерело істини для всіх локацій (мережевих та локальних): прибрати дублювання налаштувань WebDAV із меню сейвів та зробити так, щоб сейви використовували загальний список джерел (Sources).
2. Забезпечити підтримку вибору будь-якого налаштованого мережевого (WebDAV, FTP, HTTP, SMB) або локального джерела для синхронізації та автосинхронізації сейвів.

### Реалізація
1. **Єдине джерело істини для джерел**:
   - Відкочено тимчасові зміни з динамічного підмішування `"WebDAV (Saves Sync)"` у `location::Load()`. Тепер загальний список джерел зберігається виключно в `locations.ini`.
   - Оновлено `GetWebdavLocations()` у `save_locations.cpp` для повернення всіх локацій із загального списку `location::Load()`.
2. **Підтримка FTP та локальних шляхів**:
   - У `download.cpp` метод `ListWebdav()` розширено: тепер при виявленні локальних шляхів (`file://`, `sdmc:/` або відносних) список файлів зчитується безпосередньо через інтерфейси Switch `FsNativeSd::OpenDirectory` та `ReadDirectory`.
   - У `ListWebdav()` додано підтримку FTP локацій: список файлів зчитується за допомогою асинхронного запиту `NLST` в `curl`.
   - У `EncodeUrl()` додано автоперетворення локальних шляхів та схем `sdmc:/` на `file://` для коректної роботи curl.
3. **Налаштування WebDAV**:
   - У `app_display_options.cpp` метод `DisplayWebdavOptions()` повністю переписано: замість полів введення сервера, логіна та пароля додано один пункт `"Sync Location"` (вибір активного джерела). У ньому користувач вибирає одну з налаштованих у `Sources` локацій (або `"None"`). Вибір зберігається у параметрі `webdav_url` в `config.ini`.
   - У `settings_menu.cpp` додано перевірку: при видаленні мережевої локації, якщо її назва збігається з активною локацією для сейвів, налаштування синхронізації автоматично скидається на `"None"`.
   - У `save_menu_ops.cpp` автосинхронізація використовує джерело, ім'я якого збережене у налаштуваннях сейвів.
4. **Збірка**:
   - Версію підвищено до `0.13.222` у `CMakeLists.txt`. Проект успішно скомпільовано під WSL.

## v0.13.221 — Покращення стабільності та виправлення крашу

### Завдання
1. Усунути Use-After-Free краш (виліт Атмосфери з fatal error) при додаванні нових мережевих джерел у файловому браузері та налаштуваннях.

### Реалізація
1. **Виправлення крашу**:
   - У `filebrowser.cpp` імпортовано `"evman.hpp"`.
   - У методі `AddNetworkLocationInteractive()` прямі виклики `on_success()` замінено на асинхронні за допомогою `evman::push()`. Це дозволяє `PopupList` протоколів коректно закритися без виникнення помилки Use-After-Free при оновленні UI на реальній консолі.
   - Версію підвищено до `0.13.221` у `CMakeLists.txt`. Проект успішно скомпільовано під WSL.

## v0.13.220 — UX-покращення джерел даних та статус-бару

### Завдання
1. Додати підтримку протоколів WebDAV, FTP, HTTP для мережевих локацій (Sources) у файловому браузері та налаштуваннях.
2. Прибрати дублювання монтування та додавання мережевих локацій з додаткових опцій файлового браузера.
3. Оновити опцію завантаження файлів на "Upload to network location".
4. Реалізувати можливість видалення мережевих локацій безпосередньо з меню налаштувань.
5. Збільшити відстань між годинником і відсотками заряду батареї у статус-барі, щоб уникнути їх злипання при 100% заряді.

### Реалізація
1. **Статус-бар**:
   - Збільшено фіксований зсув `start_x` для батареї з 64 до 94 в `menu_base.cpp` при зарядці (`charger_type != 0`) та розрядці (`charger_type == 0`).
2. **Location Core**:
   - Додано декларацію та реалізацію функції `Remove` в `location.hpp` та `location.cpp` для видалення джерел через `minIni` (`ini_puts` з `nullptr`).
3. **File Browser (UI)**:
   - Додано інтерактивну функцію `AddNetworkLocationInteractive` з підтримкою послідовного вибору протоколу (Samba (SMB), WebDAV, FTP, HTTP) та введення параметрів через екранну клавіатуру `swkbd`.
   - Вилучено пункти `"Mount"` та `"Add network location"` з додаткових опцій (`DisplayAdvancedOptions()`).
   - Додано пункт `"Add network location"_i18n` безпосередньо в кінець вибору джерел (`ShowSourcePicker()`).
   - Перейменовано опцію `"Upload"_i18n` на `"Upload to network location"_i18n` з оновленим описом і відповідними змінами в `filebrowser_ops.cpp`.
4. **Settings Menu (UI)**:
   - Реалізовано нову категорію `"Sources"_i18n` у меню налаштувань, яка відображає список поточних мережевих джерел з можливістю видалення через `OptionBox` підтвердження та додавання нового джерела через `+ Add network location`.
5. **Локалізація**:
   - Додано переклади нових рядків в `en.json` та `uk.json`.
6. **Збірка**:
   - Версію підвищено до `0.13.220`. Проект успішно скомпільовано під WSL.
...
### Завдання
Завершити інтеграцію перевіреного каталогу з Module Manager: прибрати окремий JSON із вшитими перекладами, використовувати штатні мовні файли та забезпечити онлайн-оновлення з надійним офлайн-fallback.

### Реалізація
1. Видалено `DEFAULT_MODULES_JSON`, визначення мови всередині меню та створення `/config/kefir/modules.json`.
2. Module Manager завжди завантажує перевірені метадані з `romfs:/modules/homebrew_sysmodules.json`, а за наявності накладає на них валідний SD-індекс `/config/kefir/cache/homebrew_sysmodules.txt`.
3. Після відкриття меню актуальний список асинхронно завантажується безпосередньо з активного Gist ndeadly. Завантаження відбувається в `.download`; лише валідний список із щонайменше 50 Title ID замінює кеш. Помилка мережі не блокує й не ламає меню.
4. Описи 17 verified-модулів перенесено до штатних `assets/romfs/i18n/en.json` та `uk.json` із ключами `module.<TITLE_ID>.description`.
5. Система i18n отримала fallback до вбудованої англійської локалізації для стабільних неангломовних ключів, яких ще немає в інших перекладах.
6. Додано автоматичні перевірки узгодженості runtime-каталогу, English candidates та обох готових локалізацій, а також відсутності старого механізму описів.
7. Версію програми підвищено до `0.13.217`; повна WSL-збірка завершилася успішно.
