...
## v0.13.221 — Спільні джерела та покращення стабільності

### Завдання
1. Усунути Use-After-Free краш (виліт Атмосфери з fatal error) при додаванні нових мережевих джерел (Samba, WebDAV, FTP, HTTP) у файловому браузері та налаштуваннях.
2. Реалізувати спільні джерела: інтегрувати налаштований у сейвах WebDAV сервер до загального списку джерел (Sources) програми, щоб він автоматично відображався у файловому менеджері та налаштуваннях джерел.

### Реалізація
1. **Виправлення крашу**:
   - У `filebrowser.cpp` імпортовано `"evman.hpp"`.
   - У методі `AddNetworkLocationInteractive()` прямі виклики `on_success()` замінено на асинхронні за допомогою `evman::push(evman::FunctionalEventData { ... })`. Це дозволяє `PopupList` протоколів коректно завершити свою роботу та закритися через `SetPop()` без Use-After-Free крашу під час перемальовування UI на реальній консолі.
2. **Спільні джерела**:
   - У `location.cpp` метод `Load()` тепер автоматично додає віртуальний запис `"WebDAV (Saves Sync)"` з URL та обліковими даними з налаштувань сейвів, якщо вони сконфігуровані (`App::GetWebdavUrl()` не порожнє). Це робить джерело сейвів доступним для завантаження файлів у файловому менеджері та відображає його в загальних налаштуваннях.
   - У `settings_menu.cpp` метод `BuildSourcesCategoryItems()` оновлено: при спробі видалення джерела з назвою `"WebDAV (Saves Sync)"` програма автоматично скидає налаштування WebDAV для сейвів у `config.ini` (`App::SetWebdavUrl("")` тощо) з виведенням повідомлення `"Location deleted successfully!"`.
3. **Збірка**:
