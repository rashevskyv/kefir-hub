...
* У меню Tools тепер відображаються лише початкові 9 плиток. Натискання кнопки `Start` (або `X`) відкриває вікно `Network Server` з вибором "Web Server" або "Screenshots".
* Шлях галереї скріншотів змінено на `/album`.
* Зворотні слеші у JS успішно вичищені, черга аплоаду в браузері клієнта тепер працює справно.
...
* **Виправлення витоку дискового простору (A2)**:
  ...
  * У разі будь-якого виходу з функції з помилкою або скасуванням (коли прапорець `success` лишається `false`), деструктор автоматично видаляє недописаний файл за допомогою `fs.DeleteFile(out_path)`.
...
### Результати тестування
* Код успішно компілюється через WSL.
* Ресурси та мертвий код успішно очищені.
* Скасування аплоаду або обрив з'єднання більше не залишає недописаних файлів-пустушок на SD-карті.
* Галерея скріншотів працює швидше за рахунок усунення повторної ініціалізації title на кожен запит.
