...
> **Не збиралося (WSL build error) та суперседено у v0.13.156:** підпис `"SD: ..."` і горизонтальний скрол довгого шляху (пункт "Довгий шлях у рядку Location" нижче) замінено на алгоритм 2-рядкового розподілу та мітку `sd://` — див. розділ v0.13.156 вище. Записи цього розділу лишаються як історія рішень, а не поточна поведінка.
