...
- У цьому libnx syscall має 4 аргументи. Handle — `INVALID_HANDLE`.
...
- «Пропустити це оновлення» на всю ширину над Пізніше / Оновити, текст по центру кнопки. **Minus** — скіп, **Plus** — оновити, **B** — пізніше.
...
- Settings: **Автооновлення** — папка зверху в **Загальний**, не окремий розділ зліва.
...
- У текстовому редакторі **Y** перемикає `=0`/`=1` (і true/false / u8!0x0). **A** знову відкриває клавіатуру рядка.
...
- Окрему сторінку редактора прибрано. Файл і вставка під курсор — той самий remote input (textarea, Send).
...
- Відкрити текстовий файл → **+** Options → «Редагувати на ПК / телефоні». QR або URL. У браузері простий редактор (підсвітка, Ctrl+Z/X/C/V, Ctrl+S). Save пише файл на карту.
...
- Контекстне меню пікера: створити теку і закрити вибір папки (повертає до розпакування).
...
- Лінії між файлами в дереві більше не стрибають за курсором.
- Рамка вибору списку дій стоїть між золотими смугами, не на них і не під футером.
...
- Автофорвардер знову дозволяє скріншоти і запис відео.
- `https://https://…` після вставки згортається до одного схеми (браузер і консоль).
- Невдалий URL: текст без коду 0x, кнопка «Виправити URL» відкриває той самий prompt.
