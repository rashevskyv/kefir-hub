...
# План: Інтерактивне керування скрінсейвером аналоговими стіками (v0.13.430)

1. **Керування лівим стіком (вільне переміщення)**:
   - Додано опитування `m_stick_l` в `App::Poll()` та оновлення координат у `Screensaver::Update()`.
   - Гравець/користувач може вільно переміщати весь блок скрінсейверу по екрану Switch як у грі з плавною швидкістю та обмеженням меж екрану.

2. **Керування правим стіком (яскравість та швидкість)**:
   - Вгору / Вниз: інтерактивна плавна зміна яскравості екрана в реальному часі через `lblSetCurrentBrightnessSetting` / `lblApplyCurrentBrightnessSettingToBacklight` та синхронізація з налаштуваннями.
   - Вліво / Вправо: зміна швидкості автоматичного дрейфування скрінсейверу (`m_drift_speed_mult`).

3. **Логіка пробудження (WantsWake)**:
   - Аналогові стіки оновлюють лише зміщення та параметри рендеру і не тригерять `m_kdown`, дозволяючи інтерактивне керування без виходу зі скрінсейверу.
   - Натискання будь-якої кнопки (A/B/X/Y/D-pad) або дотик до екрану миттєво пробуджує консоль.

## Верифікація

- [x] Оновлено `types.hpp`, `app.cpp`, `screensaver.hpp`, `screensaver.cpp`, `dbi_menu.cpp`.
- [x] Версію в `CMakeLists.txt` піднято до `0.13.430`.
