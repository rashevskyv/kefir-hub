...
## Попередній delivery: v0.13.598 (svcGetSystemInfo Handle)
...
## Попередній delivery: v0.13.596 (Remote editor closed overlay)
...
## Попередній delivery: v0.13.589 (Games list badges match icon layouts)
...
- [x] `DOCS-BUMP-582` — версію піднято до `0.13.582`. Агент не компілював.

## Попередній delivery: v0.13.581 (Deploy; only-if-newer)

- [x] `UPDATE-FORCE-OFF-581` — `kForceUpdateForTest = false`.
- [x] `DOCS-BUMP-581` — версію піднято до `0.13.581`.

## Попередній delivery: v0.13.580 (Ask dialog Skip row + Minus)
...
## Попередній delivery: v0.13.578 (Ask: Later / Skip this update / Update)
...
## Попередній delivery: v0.13.575 (About markdown headings + update button)
...
## Попередній delivery: v0.13.572 (Auto-update settings category)
...
## Попередній delivery: v0.13.568 (Non-silent update uses download icon)
...
## Попередній delivery: v0.13.566 (Auto-update modes and header progress)

- [x] `UPDATE-VPREFIX-566` — порівняння версії знімає префікс `v` з tag_name.
- [x] `UPDATE-MODES-566` — Off / Silent / Ask / On demand + Update now.
- [x] `UPDATE-HEADER-566` — під час завантаження хедер: «Оновлення» + бар; без бейджа EmuNAND.
- [x] `DOCS-BUMP-566` — версію піднято до `0.13.566`, оновлено plan/task/walkthrough/audit. Агент не компілював.

## Попередній delivery: v0.13.565 (Text file open menu + range glyphs)
...
## Попередній delivery: v0.13.562 (Remote editor fills the window)

- [x] `REMOTE-EDITOR-FULLHEIGHT-562` — CodeMirror на всю висоту вікна, не дефолтні 300px з CDN CSS.
- [x] `DOCS-BUMP-562` — версію піднято до `0.13.562`, оновлено plan/task/walkthrough/audit. Агент не компілював.

## Попередній delivery: v0.13.561 (File browser: Edit on PC / phone)
...
## Попередній delivery: v0.13.556 (Paste snippet from PC)
...
## Попередній delivery: v0.13.554 (Picker folder name from archive)
...
## Попередній delivery: v0.13.552 (Picker Close in Options)

- [x] `PICKER-CLOSE-OPTION-552` — у Options пікера пункти Create Folder і Close picker.
- [x] `DOCS-BUMP-552` — версію піднято до `0.13.552`, оновлено plan/task/walkthrough/audit. Агент не компілював.

## Попередній delivery: v0.13.551 (Folder picker extract UX)

- [x] `PICKER-CREATE-FOLDER-551` — у пікері Options лише Create Folder, без copy/cut.
- [x] `PICKER-BACK-TO-EXTRACT-551` — «−» закриває пікер, не весь Direct Download; екран розпакування лишається під ним.
- [x] `DOCS-BUMP-551` — версію піднято до `0.13.551`, оновлено plan/task/walkthrough/audit. Агент не компілював.

## Попередній delivery: v0.13.550 (Zip extract separators / outline)
...
- [x] `DOCS-BUMP-549` — версію піднято до `0.13.549`, оновлено plan/task/walkthrough/audit. Агент не компілював.

## Попередній delivery: v0.13.548 (Delete our previous auto-forwarder)

- [x] `FORWARDER-STALE-OWN-548` — при встановленні нового Kefir Hub ікона видаляти попередні свої 0x05 (не HBL). Не чіпати title, з якого зайшли.
- [x] `FORWARDER-TID-HASH-548` — auto-install більше не ставить args=path, тож TID збігається з перевіркою IsInstalled.
- [x] `DOCS-BUMP-548` — версію піднято до `0.13.548`, оновлено plan/task/walkthrough/audit. Агент не компілював.

## Попередній delivery: v0.13.547 (Launch app after zip install)
...
## Попередній delivery: v0.13.543 (Centered OptionBox buttons, honest forwarder notices)
...
- [x] `DIRECT-DL-FRIENDLY-ERROR-542` — замість ErrorBox: пояснення і «Edit URL».
- [x] `DOCS-BUMP-542` — версію піднято до `0.13.542`, оновлено plan/task/walkthrough/audit. Агент не компілював.

## Попередній delivery: v0.13.541 (Install NRO to /switch label)
