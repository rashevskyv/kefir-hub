...
- [x] FIX-MTP-NO-CANCEL-373 — лог довів провину cancel-рукостискання: після
  ...
  переперелічення.
- [x] FIX-MTP-NO-OPEN-PHASE-374 — прибирання cancel спрацювало: 68 MiB на
  повній швидкості, реконект відпрацював 3 рази. Але падіння лишилось, і
  його форма назвала причину: читання йшли 26-38/с аж до самого збою, тобто
  лінк був живий — змінилось те, що пайплайн yati наповнився і потік читання
  почав чекати на запис у NAND. Data phase, недочитана в момент паузи
  споживача, отримує STALL. Тепер стрім = рівно запит виклику, тобто фаза
  завжди завершується всередині того ж читання. Простій шини МІЖ
  транзакціями — штатний MTP. Додано лічильники (commands/MiB/ms від
  конекту) у лог при кожному збої. Ретраї: 400/800/1200 мс замість 100.
- [x] FIX-USB-PACKET-ALIGNMENT-375 — 425 MiB (найдалі за весь час), але ~44
  зриви по дорозі. Лічильники з .374 спростували всі теорії «телефон
  втомлюється»: сесії помирали після 20-70 команд, 0-24 MiB і 41-4161 мс —
  жодного сталого ліміту, тобто нічого не вичерпується. Підвисання «кожні
  2 секунди», які бачить користувач, — це і є цикли реконекту.
  Стале інше: збій ЗАВЖДИ на 28-байтовому пості команди, тобто стан ламався
  ще до запиту. Причина — перезапис пам'яті: усі IN-пости вирівнювались на
  512, і `ReadStream::carry` був такого ж розміру, але консоль піднімає
  USB 3.0, де bulk wMaxPacketSize = 1024. Пост, не кратний реальному
  розміру пакета, лишає контролер без місця під хвіст останнього пакета, а
  будь-який трансфер із overshoot > 512 писав memcpy за межі `carry[512]`
  у решту стану стріму. Тепер вирівнювання по `wMaxPacketSize` з
  дескриптора (читається при відкритті та пишеться в лог), `carry` — ціла
  сторінка, обидва memcpy клампляться її розміром, заголовок читається
  посторінковим постом (кратний будь-якому легальному пакету).
- [~] FIX-USB-PACKET-ALIGNMENT-375 — СПРОСТОВАНО логом: `bulk in max packet:
  512`, телефон працює на High Speed, не USB 3.0. Вирівнювання й так було
  правильне, `carry` ніколи не переповнювався. Зміна лишається як
  необхідне зміцнення (клампи + читання розміру з дескриптора), але це
  НЕ була причина.
- [x] FIX-USB-RETRY-IN-PLACE-376 — два реальні відхилення від libusbhsfs,
  драйвера, що ганяє гігабайти через той самий usb:hs:
  1. endpoint відкривався з резервом 1 MiB на urb; libusbhsfs передає
     `wMaxPacketSize` і крізь нього постить 8 MiB — тобто це резерв на urb,
     а не стеля посту. Тим більше що usb:hs спільний з libusbhsfs, який у
     цьому ж застосунку теж ініціалізований. Тепер збігається.
  2. невдалий пост не мав локального відновлення — єдиним виходом було
     рвати сесію, переперелічуватись і перечитувати корінь. libusbhsfs
     знімає stall і одразу повторює той самий трансфер; `PostBuffer` тепер
     теж. Це і є відповідь на «інтерфейс заморожується»: `devoptab_read`
     тримає лок маунта, і файловий браузер за вікном інсталяції блокується
     на весь реконект. Backoff зменшено 400/800/1200 → 150/300/450 мс.
- [x] FIX-USB-RETRY-EVEN-HEALTHY-377 — лог 16:29 звів усі зриви до одного:
  перший OUT-пост після 1-2.5 с простою шини падає 2140-0301 (0/28) БЕЗ
  halt; та сама команда всередині безперервного трафіку завжди проходить
  (рядок 174 vs 135 — прямий A/B). Ретрай з .376 не спрацьовував жодного
  разу, бо вимагав halted — це був неточний порт: libusbhsfs знімає halt
  лише коли він є, але повторює трансфер завжди, якщо сам GET_STATUS
  вдався. Тепер збігається; контрольний запит і є тим поштовхом, що будить
  простояний лінк перед другою спробою.
- [x] FIX-RECONNECT-RESILIENCE-378 — .377 працює: 5 відновлень проти 90 у
  минулому лозі, ретрай поглинає майже всі idle-стійли. Але ран усе одно
  впав через три дрібниці разом: (1) ретрай-цикл читання після сну кликав
  EnsureSession один раз і здавався з першої ж невдачі — тепер витрачає всі
  спроби на реанімацію; (2) `last_recover_tick` переживав CloseUsb, і
  свіжа сесія з першим же збоєм OpenSession порівнювала застарілий тік з
  last_ok_tick і рвала лінк до своєї єдиної дозволеної спроби відновлення —
  тепер тік скидається при відкритті сесії; (3) «endpoint halt could not
  be cleared» ховав, який саме ctrl-трансфер і з яким кодом упав — тепер
  обидва шляхи логують endpoint і Result.
- [ ] HW-SMOKE-378 — встановлення NSP з телефону до кінця.
